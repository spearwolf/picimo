...
describe('ChunkQuadTreeNode', () => {
  describe('create without children', () => {
    ...
    it('is instance of ChunkQuadTreeNode', () =>
      assert(node instanceof ChunkQuadTreeNode));
  ...
  describe('create with irregular chunks', () => {
    ...
    it('chunk->B->containsTileIdAt(5, 6)', () =>
      assert.equal(chunks.B.containsTileIdAt(-5, -10), true));
    it('chunk->B->containsTileIdAt(5, 9)', () =>
      assert.equal(chunks.B.containsTileIdAt(0, -10), false));
    it('chunk->B->containsTileIdAt(5, 6)', () =>
      assert.equal(chunks.B.containsTileIdAt(-2, -6), true));
    it('chunk->B->containsTileIdAt(5, 9)', () =>
      assert.equal(chunks.B.containsTileIdAt(-6, -6), false));
  ...
  describe('create with grid aligned chunks', () => {
    ...
    it('find chunks contained: (2, 4)[6, 4]', () => {
      assert.deepEqual(
        chunkNodeNames(node.findVisibleChunks(new AABB2(2, 4, 6, 4))),
        ['K', 'L', 'O', 'P'].sort(),
      );
    });
    ...
    it('find chunks contained: (-2, -2)[5, 5]', () => {
      assert.deepEqual(
        chunkNodeNames(node.findVisibleChunks(new AABB2(-2, -2, 5, 5))),
        ['F', 'G', 'J', 'K'].sort(),
      );
    });
    ...
    it('find chunks contained: (-9, -8)[2, 2]', () => {
      assert.deepEqual(
        chunkNodeNames(node.findVisibleChunks(new AABB2(-9, -8, 2, 2))),
        ['A'].sort(),
      );
    });
