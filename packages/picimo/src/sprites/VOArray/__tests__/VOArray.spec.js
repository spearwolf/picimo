...
describe('VOArray', () => {
  it('create instance', () => {
    ...
    assert.strictEqual(
      voa.serial,
      1,
      `serial should be initialized with 1, but is ${voa.serial}`,
    );
  });
  ...
  it('subarray() should create a new VOArray using the same internal buffer', () => {
    ...
    assert.strictEqual(
      ...
      3 * 16 * Uint32Array.BYTES_PER_ELEMENT,
    );
  ...
  it('copy() with offset', () => {
    ...
    const b = new VOArray(4, 16 * Uint32Array.BYTES_PER_ELEMENT, [
      'uint8',
      'float32',
      'uint32',
    ]);
  ...
  it('toUint32Array() should create the uint32Array property', () => {
    ...
    const voa = new VOArray(
      2,
      16 * Uint16Array.BYTES_PER_ELEMENT,
      ['uint16'],
      values,
    );
    ...
    assert.equal(
      ...
      true,
      'toUint32Array() should return an instance of Uint32Array',
    );
    assert.ok(voa.uint32Array, 'uint32Array should be defined now');
    assert.strictEqual(
      voa.toUint32Array(),
      arr,
      'a subsequent call to toUint32Array() should return the cached instance',
    );
  });
});
