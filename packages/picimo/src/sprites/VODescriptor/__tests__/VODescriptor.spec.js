...
describe('VODescriptor', () => {
  ...
  it('attribute: position', () => {
    assertVOAttrDescriptor(descriptor, 'position', 3, 0, false, [
      'x',
      'y',
      'z',
    ]);
  ...
  it('attribute: uv', () => {
    assertVOAttrDescriptor(descriptor, ['uv', 'texCoords'], 2, 4, false, [
      's',
      't',
    ]);
  });
  ...
  describe('Vertex Object', () => {
    ...
    it('properties: texCoords', () => {
      ...
      assertPropertyTypes(vo, {
        ...
        number: ['s0', 't0', 's1', 't1', 's2', 't2', 's3', 't3'],
        undefined: [
          'getTexCoords',
          'texCoords_00',
          'texCoords_10',
    ...
    describe('attr descriptor value setter', () => {
      ...
      it('setValue(size>=2, uniform)', () => {
        ...
        assert.deepEqual(
          vo.toArray(attrList),
          [0, 0, 7, 9, 0, 0, 0, 7, 9, 0, 0, 0, 7, 9, 0, 0, 0, 7, 9, 0],
          'attr.setValue()',
        );
      });
      ...
      it('setValue(size>=2)', () => {
        ...
        assert.deepEqual(
          vo.toArray(attrList),
          [0, 1, 2, 0, 0, 0, 3, 4, 0, 0, 0, 5, 6, 0, 0, 0, 7, 8, 0, 0],
          'attr.setValue()',
        );
      });
    });
    ...
    it('internal array buffer', () => {
      ...
      // x
      assert.equal(
        descriptor.attr.position.getValue(vo, 0, 0),
        1,
        'position.getValue(0, 0)',
      );
      assert.equal(
        descriptor.attr.position.getValue(vo, 1, 0),
        4,
        'position.getValue(1, 0)',
      );
      assert.equal(
        descriptor.attr.position.getValue(vo, 2, 0),
        7,
        'position.getValue(2, 0)',
      );
      assert.equal(
        descriptor.attr.position.getValue(vo, 3, 0),
        10,
        'position.getValue(3, 0)',
      );

      // y
      assert.equal(
        descriptor.attr.position.getValue(vo, 0, 1),
        2,
        'position.getValue(0, 1)',
      );
      assert.equal(
        descriptor.attr.position.getValue(vo, 1, 1),
        5,
        'position.getValue(1, 1)',
      );
      assert.equal(
        descriptor.attr.position.getValue(vo, 2, 1),
        8,
        'position.getValue(2, 1)',
      );
      assert.equal(
        descriptor.attr.position.getValue(vo, 3, 1),
        11,
        'position.getValue(3, 1)',
      );

      // z
      assert.equal(
        descriptor.attr.position.getValue(vo, 0, 2),
        3,
        'position.getValue(0, 2)',
      );
      assert.equal(
        descriptor.attr.position.getValue(vo, 1, 2),
        6,
        'position.getValue(1, 2)',
      );
      assert.equal(
        descriptor.attr.position.getValue(vo, 2, 2),
        9,
        'position.getValue(2, 2)',
      );
      assert.equal(
        descriptor.attr.position.getValue(vo, 3, 2),
        12,
        'position.getValue(3, 2)',
      );
      ...
      assert.equal(
        descriptor.attr.rotate.getValue(vo),
        90,
        'rotate.getValue()',
      );
      ...
      assert.equal(
        descriptor.attr.translate.getValue(vo, 0, 0),
        42,
        'translate.getValue(0, 0)',
      );
      assert.equal(
        descriptor.attr.translate.getValue(vo, 0, 1),
        43,
        'translate.getValue(0, 1)',
      );
...
describe('VODescriptor with mixed element types (using the attribute object notation)', () => {
  ...
  it('.typedArrays', () => {
    ...
    assert.equal(descriptor.typedArrays.float32, true, 'typedArrays.float32');
    assert.deepEqual(
      descriptor.typeList.sort(),
      ['uint8', 'uint16', 'float32'].sort(),
      'typeList',
    );
  });
  ...
  it('attr.type + byteOffset', () => {
    ...
    assert.equal(
      descriptor.attr.translate.byteOffset,
      16,
      'translate.byteOffset',
    );
  });
});
