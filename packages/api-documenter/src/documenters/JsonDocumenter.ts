...
export class JsonDocumenter {
  ...
  /** @virtual */
  protected onCustomizeYamlItem(_yamlItem: IYamlItem): void {
    // virtual
          ...
          case FlattenMode.NestedNamespacesAndChildren:
            ...
            if (
              !this._flattenNamespaces(
                item.members,
                childrenOut,
                FlattenMode.NestedNamespacesOnly,
              )
            ) {
  ...
  private _writeTocFile(apiItems: ReadonlyArray<ApiItem>): void {
    ...
    const tocFilePath: string = path.join(this._outputFolder, 'sidebars.json');
    console.log(`Writing ${tocFilePath}`);

    FileSystem.writeFile(tocFilePath, JSON.stringify(tocFile, null, 2), {
  ...
  /** @virtual */
  protected buildJsonTocFile(apiItems: ReadonlyArray<ApiItem>): Array<unknown> {
    const tocItems = this._buildTocItems(apiItems);
    return tocItems;
  }

  private _buildTocItems(apiItems: ReadonlyArray<ApiItem>) {
  ...
  private _generateYamlItem(apiItem: ApiDocumentedItem): IYamlItem | undefined {
    ...
    if (apiItem.kind !== ApiItemKind.Namespace && apiItem.parent) {
    ...
    switch (apiItem.kind) {
      ...
      case ApiItemKind.Interface:
        ...
        this._populateYamlClassOrInterface(
          uid,
          yamlItem,
      ...
      case ApiItemKind.MethodSignature:
        ...
        this._populateYamlFunctionLike(
          uid,
          yamlItem,
          apiItem as ApiMethod | ApiMethodSignature,
        );
        break;
  ...
  private _populateYamlTypeParameters(
    ...
    for (const apiTypeParameter of apiItem.typeParameters) {
      ...
      if (!apiTypeParameter.constraintExcerpt.isEmpty) {
        typeParameter.type = [
          this._renderType(contextUid, apiTypeParameter.constraintExcerpt),
        ];
      }
  ...
  private _populateYamlClassOrInterface(
    ...
    if (apiItem instanceof ApiClass) {
      if (apiItem.extendsType) {
        ...
        yamlItem.inheritance = this._renderInheritance(uid, [
          apiItem.extendsType,
        ]);
      }
      if (apiItem.implementsTypes.length > 0) {
        ...
        for (const implementsType of apiItem.implementsTypes) {
          yamlItem.implements.push(
            this._renderType(uid, implementsType.excerpt),
          );
        }
      }
      ...
      if (apiItem.extendsTypes.length > 0) {
        ...
        yamlItem.inheritance = this._renderInheritance(
          uid,
          apiItem.extendsTypes,
        );
      }

      const typeParameters: IYamlParameter[] = this._populateYamlTypeParameters(
        uid,
        apiItem,
      );
  ...
  private _populateYamlFunctionLike(
    ...
    if (ApiReturnTypeMixin.isBaseClassOf(apiItem)) {
      const returnType: string = this._renderType(
        uid,
  ...
  protected _getApiDomain(apiItem: ApiItem) {
    const item: {docComment?: string} = {};
    apiItem.serializeInto(item as any);
    const m = item.docComment?.match(/@apiDomain\s+(\w+)/);
    if (m) return m[1];
    return '';
  }
  ...
  private _initApiItemsRecursive(apiItem: ApiItem): void {
    if (apiItem.canonicalReference && !apiItem.canonicalReference.isEmpty) {
      this._apiItemsByCanonicalReference.set(
  ...
  private _renderInheritance(
    ...
    for (const heritageType of heritageTypes) {
      ...
      const apiItem:
        | ApiItem
        | undefined = this._apiItemsByCanonicalReference.get(type);
      if (apiItem) {
        if (apiItem instanceof ApiClass) {
          if (apiItem.extendsType) {
  ...
  private _renderType(
    ...
    if (
      !excerptTokens.some(
        (tok) =>
          tok.kind === ExcerptTokenKind.Reference && !!tok.canonicalReference,
      )
    ) {
      return typeName;
    }
    ...
    const existingUid: string | undefined = yamlReferences.typeNameToUid.get(
      typeName,
    );
    ...
    if (
      ...
      const apiItem:
        | ApiItem
        | undefined = this._apiItemsByCanonicalReference.get(excerptRef);
      return this._recordYamlReference(
        ...
        apiItem
          ? this._getYamlItemName(apiItem, {includeNamespace: true})
          : typeName,
      );
    }
    ...
    const counter: number =
      yamlReferences.uidTypeReferenceCounters.get(baseUid) || 0;
  ...
  private _getYamlItemName(
    ...
    if (
      ...
      for (
        let current: ApiItem | undefined = apiItem.parent;
        current;
