...
class JsonDocumenter {
    ...
    _renderType(contextUid, typeExcerpt) {
        ...
        if (excerptTokens.length === 1 &&
            ...
            return this._recordYamlReference(yamlReferences, excerptTokens[0].canonicalReference.toString(), apiItem ? this._getYamlItemName(apiItem) : typeName, apiItem
    ...
    _getYamlItemName(apiItem, options = {}) {
        ...
        const baseName = includeSignature
            ? Utilities_1.Utilities.getConciseSignature(apiItem)
            : apiItem.displayName;
    ...
    _getYamlFilePath(apiItem) {
        ...
        for (const current of apiItem.getHierarchy()) {
            switch (current.kind) {
                ...
                default:
                    if (current.parent &&
                        current.parent.kind === "EntryPoint" /* EntryPoint */) {
                        result += '/';
                    }
                    else {
                        result += '.';
