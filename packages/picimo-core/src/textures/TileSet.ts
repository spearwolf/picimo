...
export class TileSet implements ITileSet {
  ...
  static async load(path: string, options?: TileSetLoadOptions) {
    ...
    const baseTexture = new Texture(await new PowerOf2Image(`${basePath}${path}`).loaded);
  ...
  randomFrames(count: number) {
    ...
    for (let i = 0; i < count; i++) {
      frames.push(this[$textures].get(this.firstId + Math.floor(Math.random() * this.tileCount)));
    }
    return frames;
  }
